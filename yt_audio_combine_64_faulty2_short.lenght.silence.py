...
def design_brickwall_fir(pass_type: str, freq: float, fs: int,
    ...
    Returns 1-D float64 tap array of odd length.
    """
    nyq = fs / 2.0
...
class PacketWeightProvider:
    ...
    def get_weights(self, start_sample: int, num_samples: int,
        ...
        """Return (w_m4a, w_opus) as (num_samples, 1) float64 arrays."""
...
def process_chunk(m4a_raw: np.ndarray, opus_raw: np.ndarray,
    ...
    m4a_raw, opus_raw: (M, 2) float64 stereo.
    Returns (M, 2) float64 stereo output.
...
class EncoderWriter:
    ...
    def write(self, chunk: np.ndarray) -> None:
        """Write (M, 2) float64 chunk to encoder stdin."""
        raw = chunk.astype(np.float64).tobytes()
        try:
...
def _read_chunk(pipe, chunk_size: int) -> np.ndarray | None:
    """Read one chunk of interleaved stereo float64 from an ffmpeg pipe.

    Returns (M, 2) float64 or None on EOF.
    """
